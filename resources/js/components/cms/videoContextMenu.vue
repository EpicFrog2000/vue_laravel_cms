...
function loadElementData() {
	...
	video_element_values.value = {
		...
		...(element ? {
			...
			poster: element.getAttribute('poster') || defaultVideoValues.poster,
			preload: (element.getAttribute('preload') as 'auto' | 'metadata' | 'none') || defaultVideoValues.preload,
		} : {})
	};
}
...
<Dialog v-model:visible="visible" modal header="Zmiana wideo">
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center gap-4">
            ...
            <Checkbox binary id="input-autoplay" v-model="video_element_values.autoplay"/>
        </div>
        <div class="flex items-center gap-4">
            <label for="input-muted" class="font-semibold w-24">Muted</label>
            <Checkbox binary id="input-muted" v-model="video_element_values.muted"/>
        </div>
        <div class="flex items-center gap-4">
            <label for="input-loop" class="font-semibold w-24">Loop</label>
            <Checkbox binary id="input-loop" v-model="video_element_values.loop"/>
        </div>
        <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-4">
            <label for="input-preload" class="font-semibold w-24">Preload</label>
            <Select id="input-preload" class="flex-auto" :options="['auto','metadata','none']" v-model="video_element_values.preload"/>
        </div>
