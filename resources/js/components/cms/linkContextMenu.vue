...
import { useLinkContextMenuStore } from '@/stores/linkContextMenuStore';
...
function loadElementData() {
	...
	link_element_values.value = {
		...
		...(element ? {
			...
			target: element.getAttribute('target') || defaultLinkValues.target,
    ...
    <Dialog v-model:visible="visible" modal header="Zmiana linku">
        <div  class="flex flex-col gap-4">
            <div class="flex items-center gap-4">
                <label for="input-href" class="font-semibold w-24">Href</label>
                <InputText id="input-href" class="flex-auto" autocomplete="off" v-model="link_element_values.href"/>
            </div>
            <div class="flex items-center gap-4">
                <label for="input-text" class="font-semibold w-24">Link text</label>
                <InputText id="input-text" class="flex-auto" autocomplete="off" v-model="link_element_values.link_text"/>
            </div>
            <div class="flex items-center gap-4">
                <label for="input-target" class="font-semibold w-24">Target</label>
                <Select id="input-target" class="flex-auto" :options="['_self','_blank','_parent','_top']" v-model="link_element_values.target"/>
            </div>
