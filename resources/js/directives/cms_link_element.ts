...
import { actionResponse, CmsLink, defaultLinkValues } from '@/types/cmsTypes';
...
export default function createLinkCmsElement(auth:boolean): Directive {
    return {
...
function getStructure() {
    ...
    for (const el of cms_elements) {
        ...
        ref[finalKey] = <CmsLink>{
            ...defaultLinkValues,
            href: decodedValue(el.href),
            link_text: decodedValue(el.innerHTML),
            target: decodedValue(el.getAttribute('target') ?? ''),
        };
    }
...
export async function updatecms_link_values(): Promise<actionResponse> {
