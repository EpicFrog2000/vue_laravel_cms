...
import { actionResponse, CmsVideo, defaultVideoValues } from '@/types/cmsTypes';
...
export default function createVideoCmsElement(auth:boolean): Directive {
    return {
        mounted: async (el: CmsElement, binding: any) => {
            ...
            const showBorder = (e: KeyboardEvent) =>
                e.ctrlKey &&
                (el.style.boxShadow = '0 0 6px 2px rgba(125,0,255,0.7)');
...
function getStructure() {
    ...
    for (const el of cms_elements) {
        ...
        ref[finalKey] = <CmsVideo>{
            ...
            preload:
                (el.getAttribute('preload') as 'auto' | 'metadata' | 'none') ||
                'auto',
        };
    }
